...
* **Human-in-the-Loop Registration Checkpoint**: Execution outputs from the Graph Engine are strictly isolated in a frontend staging layer. A human operator must explicitly review and authorize the "Register Obligation" action before the regulation is committed to PostgreSQL and routed to live department queues, completely blocking unreviewed AI hallucinations.
...
* **Advanced Compliance Reporting & Heatmaps**: Built-in Admin tools feature a live 2D Compliance Risk Heatmap to visualize departmental loads, dynamic department filtering, a Global Omni-Search engine, and one-click CSV Data Exports of the entire compliance queue.
